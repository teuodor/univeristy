    ...
    public class Thr1
    {
        ...
        public void run()
        {
            try
            {
                ...
                sqlCon.Open();
                SqlCommand cmd = new SqlCommand("Dead1", sqlCon);
            ...
            catch (SqlException)
            {
                ...
                if (rdr != null)
                    rdr.Close();
                tries--;
                Console.WriteLine("Thread 1 dead, " + tries + " more tries left");
                if (tries > 0)
                    run();
            }
        }
    ...
    public class Thr2
    {
        ...
        public void run()
        {
            try
            {
                ...
                sqlCon.Open();
                SqlCommand cmd = new SqlCommand("Dead2", sqlCon);
                cmd.CommandType = CommandType.StoredProcedure;
                rdr = cmd.ExecuteReader();
                sqlCon.Close();
                rdr.Close();
            ...
            catch (SqlException)
            {
                ...
                if (rdr != null)
                    rdr.Close();
                tries--;
                Console.WriteLine("Thread 2 dead, " + tries + " more tries left");
                if (tries > 0)
                    run();
            }
        }
