...
void testCreeaza() {
	...
	IteratorContainer ic = c.iterator(); //iterator pe colectia vida ar trebui sa fie invalid din start
	assert(ic.valid() == false);
}

void testAdauga() {
	Container c; //adaugam elemente [0, 10)
	for (int i = 0; i < 10; i++) {
		c.adauga(i);
	}
	assert(c.vida() == false);
...
void testIterator() { // nu stim reprezentarea Colectiei, putem testa doar anumite lucruri generale, nu stim in ce ordine vor fi afisate elementele.
	Container c;
	IteratorContainer ic = c.iterator(); //iterator pe colectie vida
	assert(ic.valid() == false);

	for (int i = 0; i < 100; i++) {  //adaug 100 de elemente, fiecare e valoarea 33
		c.adauga(33);
	}
	IteratorContainer ic2 = c.iterator(); //daca iterez doar 33 poate sa-mi dea iteratorul
	...
	IteratorContainer ic3 = c2.iterator();
	...
	IteratorContainer ic4 = c3.iterator();
	...
	assert(count == 50);
	Container c4; // adaugam niste elemente in colectie
	for (int i = 0; i < 100; i++) {
		c4.adauga(i);
		c4.adauga(i * (-2));
		c4.adauga(i * 2);
	...
	IteratorContainer ic5 = c4.iterator();
	while (ic5.valid()) {
		TElem e = ic5.element();
	...
	Container c5; // adaugam niste elemente in colectie
	for (int i = 0; i < 100; i++) {
		c5.adauga(i);
		c5.adauga(i * (-2));
		c5.adauga(i * 2);
	...
	IteratorContainer ic6 = c5.iterator();
	while (ic6.valid()) {
		TElem e = ic6.element();
